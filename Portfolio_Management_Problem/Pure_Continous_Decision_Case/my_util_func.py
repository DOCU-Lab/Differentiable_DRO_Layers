...
def loss(decision,y):
    cost=torch.concatenate([-y[i].reshape(1,-1) @ decision[0][i] for i in range(len(decision[0]))]).mean()
    return cost

def loss_show(decision,y):
    cost=torch.concatenate([-y[i].reshape(1,-1) @ decision[0][i] for i in range(len(decision[0]))])
    return cost
