...
class e2e_net(nn.Module):
    ...
    def __init__(self, n_x, n_y, n_obs, opt_layer='nominal', prisk='p_var', perf_loss='sharpe_loss',
        ...
        if pred_model == 'linear':
            ...
            self.pred_layer = nn.Sequential(nn.Linear(n_x, 22),
                      nn.ReLU(),
                      nn.Linear(22, 27),
                      nn.ReLU(),
                      nn.Linear(27, n_y))
    ...
    def net_train(self, train_set, val_set=None, epochs=None, lr=None):
        ...
        for epoch in range(epochs):
            ...
            for name, param in self.named_parameters():
                ...
                if name=='delta':
                    param.data.clamp_(0.0001)
            torch.save(self.gamma,f'gamma_epoch_{epoch}')
            torch.save(self.delta,f'delta_epoch_{epoch}')
            torch.save(self.pred_layer,f'pred_layer_{epoch}')
